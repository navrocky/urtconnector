...
namespace {
    ...
    struct compose{
        ...
        void operator()(){ if(f1_) f1_(); if(f2_) f2_();  if(f3_) f3_(); }
    };
    
    //invoke compose function fromm itemData
    void invoke_data( QComboBox* combo ){ combo->itemData( combo->currentIndex() ).value<compose>()(); }
}
...
ping_filter_class::ping_filter_class()
    : filter_class(get_id(), QObject::tr("Ping filter"),
        QObject::tr("Hides all servers that don's match selected ping") )
{}
